...
class ValueCritic(nengo.Network):
    def __init__(self,n_neurons_state, n_neurons_value, theta, d, discount, q_n, q_r, q_v,
        ...
        if "activity_lmu_transform" in kwargs:
            ...
            self.value_transform = kwargs.pop("value_transform")
        ...
        with self:
            ...
            nengo.Connection(self.value, self.td_error, 
...
class NeuralValueCritic(Network):
    def __init__(self,n_neurons_state, n_neurons_value, n_neurons_lmus, theta, d, discount, q_a, q_r, q_v,
        ...
        with self:
            ...
            self.state_memory = LMUNetwork(n_neurons_lmus, theta=theta, q=q_a, size_in=n_neurons_state, tau=tau)
            nengo.Connection(self.state.neurons, self.state_memory.input, synapse=tau, transform=1/1000)

            self.reward_input = nengo.Node(size_in=1)
            self.reward_memory  = LMUNetwork(n_neurons_lmus, theta=theta, q=q_r, size_in=1, tau=tau)
            ...
            self.value_memory = LMUNetwork(n_neurons_lmus, theta=theta, q=q_v, size_in=1, tau=tau)
...
class ValueDistCritic(nengo.Network):
    def __init__(self,n_neurons_state, n_neurons_value, theta, d, discount, q_a, q_r, q_v,
                 algor,  reward_ssp_space, learning_rate=1e-4, T_test=10000,state_ensemble=None,tau=0.05, lambd=0.8,
                  **kwargs):
        super().__init__()
        
        self.activity_lmu_transform, self.reward_lmu_transform, self.value_transform, self.value_lmu_transform = get_critic_transforms(algor, discount, n_neurons_state, theta,
                                                                                                                                       size=reward_ssp_space.ssp_dim,
                                                                                                                             q_a=q_a, q_r = q_r, q_v=q_v, lambd=lambd)
        ...
        with self:
            ...
            self.reward_input = nengo.Node(lambda t,x: reward_ssp_space.encode(x).flatten(), size_in=1)
            lmu_r = LMUProcess(theta=theta, q=q_r,size_in=reward_ssp_space.ssp_dim)
            self.reward_memory = nengo.Node(lmu_r)
            nengo.Connection(self.reward_input, self.reward_memory, synapse=None) 

            lmu_v = LMUProcess(theta=theta, q=q_v,size_in=reward_ssp_space.ssp_dim)
            self.value = nengo.Ensemble(n_neurons_value,reward_ssp_space.ssp_dim)
            self.value_memory = nengo.Node(lmu_v)
            nengo.Connection(self.value, self.value_memory, synapse=tau)
            self.learn_connV = nengo.Connection(self.state.neurons, self.value, 
                                                transform=np.zeros((reward_ssp_space.ssp_dim,n_neurons_state)), 
                                        learning_rule_type = DPES(reward_ssp_space.ssp_dim, n_neurons_state, 
                                                                 learning_rate = learning_rate),synapse=tau)

            self.error = nengo.Node(lambda t,x: x if t<T_test else 0, 
                               size_in=reward_ssp_space.ssp_dim + n_neurons_state)
            
            nengo.Connection(self.reward_memory, self.error[:reward_ssp_space.ssp_dim], 
                             transform=-self.reward_lmu_transform)
            nengo.Connection(self.value, self.error[:reward_ssp_space.ssp_dim], 
                             transform=-self.value_transform, synapse=0.05)
            nengo.Connection(self.value_memory, self.error[:reward_ssp_space.ssp_dim],  
                             transform=self.value_lmu_transform, synapse=None)
            nengo.Connection(self.state_memory, self.error[reward_ssp_space.ssp_dim:], 
                             transform=self.activity_lmu_transform, synapse=None)
...
##############################
# General network that learns a SR function given LDNs in nodes & decoders
#######################
...
class SRCritic(Network):
    def __init__(self,n_neurons_state, n_neurons_sr,n_neurons_r, theta, d, discount, q_n, q_s, q_sr, 
        ...
        with self:
            ...
            # The learned reward rep can depend on the 'context', could use state_memory_ens
            # For now using state 
...
def get_critic_transforms(rule_type, discount, n_neurons, theta, size=1,
    ...
    if rule_type=="TD0":
        ...
        for i in range(q_a):
            ...
            value_transform[i]=(1/theta)*integrate.quad(intgrand, 0,1,epsabs=epsabs)[0]

            for j in range(q_r):
                intgrand = lambda y,x: (1/y)*(discount**(theta*y*(1-x)))*legendre(i)(2*x-1)*legendre(j)(2*y-1)
                reward_lmu_transform[j,i]=(1/theta)*integrate.dblquad(intgrand, 0,1,lambda x: 0, lambda x: x, epsabs=epsabs)[0]
...
def get_Q_transforms(rule_type, discount, n_neurons, theta, size,
                    q_n=10, q_a=10, q_r = 10, q_v=10, alpha=10, lambd=0.8):
    ...
    if rule_type=="TD0":
        reward_lmu_transform = np.asarray([legendre(i)(1) for i in range(q_r)])
        reward_lmu_transform = np.kron(1, reward_lmu_transform.reshape(q_r, -1).T)
        
        value_transform = discount/theta
        
        value_lmu_transform = np.asarray([legendre(i)(1) for i in range(q_v)])
        value_lmu_transform = (1+discount/theta)*np.kron(1, value_lmu_transform.reshape(q_v, -1).T)
        ...
        reward_lmu_transform =  np.kron(1, reward_lmu_transform.reshape(1, -1))
        value_transform = discount**theta

        value_lmu_transform = np.asarray([legendre(i)(1) for i in range(q_v)])
        value_lmu_transform = np.kron(1, value_lmu_transform.reshape(q_v, -1).T)
        ...
        value_lmu_transform2 = np.asarray([legendre(i)(1) for i in range(q_v)])
        value_lmu_transform = np.kron(1, (value_lmu_transform1-value_lmu_transform2).reshape(q_v, -1).T)

    else:
        print("Not a valid rule")
    return activity_lmu_transform, action_lmu_transform, reward_lmu_transform, value_transform, value_lmu_transform 
