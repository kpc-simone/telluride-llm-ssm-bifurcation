...
class OfflineValueCritic(nengo.Network):
    def __init__(self,n_neurons_state, n_neurons_value, theta, d, discount, q_s, q_r, q_v,
                 algor, learn_schedule,
                 learning_rate=1e-4, T_test=10000,tau=0.05, lambd=0.8, dtau = 0.01, dt=0.001,
                 replay_type="forward",#"backward", "shuffled"
                  **kwargs):
        super().__init__()
        ...
        self.state_lmu_transform, self.reward_lmu_transform, self.value_lmu_transform, self.value_transform = get_dyn_critic_transforms(algor, discount, theta, learn_schedule[1], d,#
        ...
        with self:
            ...
            self.sampled_tau = nengo.Node(size_in=1)
            ...
            nengo.Connection(self.reset, self.state_memory[1],synapse=None) # reset after learning, make room for new experiences
            ...
            def lmu_state_learn(t,x):
                ...
                if ((t - n*(learn_schedule[0] + learn_schedule[1])) // learn_schedule[0] ) <= 0:
                    ...
                    return mat @ x#(1/learn_schedule[1])* 

            self.state_memory_input = nengo.Node(lmu_state_learn, size_in=q_s*d)
            nengo.Connection(self.state_memory, self.state_memory_input, synapse=tau)
            ...
            nengo.Connection(self.reset, self.reward_memory[1],synapse=None) # reset after learning, make room for new experiences
            ...
            nengo.Connection(self.reset, self.value_memory[1],synapse=None) # reset after learning, make room for new experiences
            ...
            def lmu_error_fun(t,x):
                ...
                if ((t - n*(learn_schedule[0] + learn_schedule[1])) // learn_schedule[0] ) <= 0:
                    ...
                    v_mat = self.reward_lmu_transform(ttau)
            ...
            # nengo.Connection(self.value, self.error[-1],synapse=tau)
...
def get_critic_transforms(rule_type, discount, theta, state_size=1, 
                    q_s=10, q_r = 10, q_v=10, alpha=10, lambd=0.8, dtau =0.1):
    
    if rule_type=="TD0":
        state_lmu_transform = np.asarray([legendre(i)(1) for i in range(q_s)])
        state_lmu_transform = np.kron(np.eye(state_size), state_lmu_transform.reshape(q_s, -1).T)
        reward_lmu_transform = np.asarray([legendre(i)(1) for i in range(q_r)]).reshape(1,-1)
        mat1 =  np.asarray([legendre(i)(1) for i in range(q_v)]).reshape(1,-1)
        mat2 =  np.asarray([legendre(i)(1-dtau) for i in range(q_v)]).reshape(1,-1)
        value_lmu_transform = (discount/dtau)*mat2 - (1 + discount/dtau)*mat1
    
    else:
        print("Not a valid rule")
    return state_lmu_transform, reward_lmu_transform, value_lmu_transform 
...
def get_dyn_critic_transforms(rule_type, discount, theta, w, state_size=1, taus= None,
    ...
    if rule_type=="TD0":
        ...
        def value_lmu_transform(tau):
            ...
            # mat1 = eval_legendre(np.arange(q_v).reshape(1,-1), tau )
            # mat2 = eval_legendre(np.arange(q_v).reshape(1,-1), tau - dtau/theta)
            # return (discount/dtau)*mat2 - (1 + discount/dtau)*mat1
            # return (discount/dtau)*eval_legendre(np.arange(q_v).reshape(1,-1), tau-theta/dtau)
        if taus is not None:
            dt = taus[1]-taus[0]
            ...
            def state_lmu_transform(tau):
                # return state_mats[np.argmin(np.abs(tau - taus))]
                return state_mats[int(tau/dt - dt)]
            def reward_lmu_transform(tau):
                # return reward_mats[np.argmin(np.abs(tau - taus))]
                return reward_mats[int(tau/dt - dt)]
            def value_lmu_transform(tau):
                # return value_mats[np.argmin(np.abs(tau - taus))]
